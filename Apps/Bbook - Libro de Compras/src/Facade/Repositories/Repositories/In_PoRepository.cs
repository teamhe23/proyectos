...
{
    public class In_PoRepository : IIn_poRepository
    {
        string ErrorMessage;
        ...
        public In_PoRepository(IDBOracleRepository iDBOracleRepository, ICommonRepository commonRepository)
        {
        ...
        //    try
        //    {
        //        return new DTOUnitario<In_Po>.Request()
        //        {
        //            data = new List<In_Po>()
        //            {
        //                GetIn_po("sp_in_po_read","")
        //            },
        ...
        {
            DTOUnitario<In_Po>.Response response = new DTOUnitario<In_Po>.Response();
            List<ErrorDTO<In_Po>> errors = new List<ErrorDTO<In_Po>>();
            pruebaInternas = new List<PruebaInterna>();
            if (_commonRepository.DataOKIn_po(ref ErrorMessage, in_PoRequest))
            {
                try
                {
                    //HACER PROCESO 
                    foreach (var item in _commonRepository.AddDataTableIn_po("In_po", "A", in_PoRequest))
                    {
                        pruebaInternas.Add(item);
                        if (item.message != "OK")
                        {
                            errors.Add(new ErrorDTO<In_Po>()
                            {
                                code = "01",
                                message = item.message,
                                record = in_PoRequest.data
                            });
                        }
                    }
                    if (errors.Count < 1) response = new DTOUnitario<In_Po>.Response() { status = "OK", statusCode = 200, internalCode = "00", message = "OK" };
                    else response = new DTOUnitario<In_Po>.Response() { status = "ERROR", statusCode = 409, internalCode = "00", message = "ERROR", errors = errors };
                }
                catch (Exception ex)
                {
                    response = new DTOUnitario<In_Po>.Response() { status = "ERROR", statusCode = 500, internalCode = "01", message = ex.Message };
                }
                finally
                {
                    _commonRepository.Bbook_history("Bbook_In_po", JsonConvert.SerializeObject(in_PoRequest),
            ...
            else
            {
                ...
                {
                    id = in_PoRequest.data.id_document == 0 ? "" : in_PoRequest.data.id_document.ToString(),
                    table = false,
                    message = ErrorMessage,
                    method = "LoadDataIn_po",
                    name = "In_po",
                    quantity = 1
                });
                response = new DTOUnitario<In_Po>.Response()
                {
                    internalCode = "99",
                    message = ErrorMessage,
                    status = "ERROR",
                    ...
                    if(ErrorMessage == "OK")
                    {
                        while (oracleDataReader.Read())
                        {
                                ...
                                Codigoactual != int.Parse(oracleDataReader["id_document"].ToString().Trim()))
                            {
                                if (tipoproceso != "-S-E")
                                {
                                    ...
                                    Out_pos.Add(new Out_Po()
                                    {
                                        ...
                                        total_units = out_PoTemporal.total_units,
                                        total_cost = out_PoTemporal.total_cost,
                                        label = labels,
                            ...
                            Out_posTemporal.Add(new Out_Po()
                            {
                                ...
                                total_units = int.Parse(oracleDataReader["total_units"].ToString().Trim()),
                                total_cost = double.Parse(oracleDataReader["total_cost"].ToString().Trim()),
